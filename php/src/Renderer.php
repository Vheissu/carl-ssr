...
final class Renderer
{
    ...
    public function __construct(array $manifest)
    {
        ...
        foreach (($manifest['components'] ?? []) as $component) {
            ...
            if (!is_string($component['template'] ?? null)) {
                throw new InvalidArgumentException("Carl component {$tagName} must define a string template.");
            }
    ...
    public static function fromManifestJson(string $json): self
    {
        $manifest = json_decode($json, true);

        if (!is_array($manifest)) {
            throw new RuntimeException('Unable to parse Carl manifest JSON.');
        }
    ...
    public static function fromManifestFile(string $path): self
    {
        ...
        return self::fromManifestJson($json);
    }
    ...
    public static function renderTemplate(string $template, array $props): string
    {
        return (string) preg_replace_callback(
            ...
            function (array $matches) use ($props): string {
                ...
                $value = self::readPath($props, $path);
    ...
    private function renderComponent(array $component, array $options): string
    {
        ...
        $body = self::renderTemplate((string) $component['template'], $props);
    ...
    /** @param array<string, mixed> $props */
    private static function readPath(array $props, string $path): mixed
    {
        $value = $props;
